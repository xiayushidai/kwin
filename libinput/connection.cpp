...
{
    qDebug()<<"this is connection";
    QMutexLocker locker(&m_mutex);
    qDebug()<<"this isempty"<<m_eventQueue.isEmpty();
    while (!m_eventQueue.isEmpty()) {
		...
		libinput_device *dev_zjg=event->nativeDevice();
                auto device = new Device(dev_zjg);
		qDebug()<<"new success";
#if 1
