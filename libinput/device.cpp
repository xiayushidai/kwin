    ...
    , m_clickMethod(libinput_device_config_click_get_method(m_device))
{
    ...
    s_devices << this;
    QDBusConnection::sessionBus().registerObject(QStringLiteral("/org/kde/zjg/InputDevice/") + m_sysName,
                                                 QStringLiteral("org.kde.zjg.InputDevice"),
                                                 this,
...
void Device::setLeftHanded(bool set)
{
    ...
    if (libinput_device_config_left_handed_set(m_device, set) == LIBINPUT_CONFIG_STATUS_SUCCESS) {
        if (m_leftHanded != set) {
            m_leftHanded = set;
            writeEntry(ConfigKey::LeftHanded, m_leftHanded);
            emit leftHandedChanged();   //发出鼠标变化的信号
        }
    }
}
